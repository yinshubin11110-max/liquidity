...
def parse_args(args: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collapsed CS spread event-study with prior Amihud heterogeneity.")
    ...
    parser.add_argument("--lookback", type=int, default=12, help="Number of months used to compute prior Amihud.")
...
def split_tickers_by_amihud(df: pd.DataFrame, lookback: int) -> Tuple[List[str], List[str]]:
    window = df[df["k"].between(-lookback, -1)]
    agg = window.groupby("ticker", observed=True)["amihud"].mean().dropna()
    if agg.empty:
        return [], []
    threshold = agg.median()
    high = agg[agg >= threshold].index.tolist()
    low = agg[agg < threshold].index.tolist()
    return high, low


def filter_group(df: pd.DataFrame, tickers: List[str], kmin: int, kmax: int) -> pd.DataFrame:
...
def fit_collapsed(df: pd.DataFrame) -> Tuple:
...
def process_group(
    ...
    subset = filter_group(df, tickers, kmin, kmax)
    if subset.empty:
        return pd.DataFrame(), 0

    design = build_design(subset)
    weights = compute_weights(design)
    base_result, terms = fit_collapsed(design)
    boot_t, post_all_t = wild_bootstrap(design, terms, base_result, B, seed, weights)
    ...
    return combined, design["ticker"].nunique()
...
def main(cli_args: Iterable[str] | None = None) -> None:
    ...
    high_tickers, low_tickers = split_tickers_by_amihud(panel_df, args.lookback)
    if not high_tickers or not low_tickers:
        raise RuntimeError("Failed to split tickers into high/low prior Amihud groups.")

    groups = {
        "HighPrior": high_tickers,
        "LowPrior": low_tickers,
    }

    notes = []
    for label, tickers in groups.items():
        combined, n_firms = process_group(
            panel_df,
            tickers,
            args.kmin,
            args.kmax,
        ...
        if combined.empty:
            raise RuntimeError(f"No valid observations for {label} group.")

        combined.to_csv(f"collapse_wild_cs_{label}.csv", index=False)
        notes.append(
            f"{label}: N_firms={n_firms}"
        )

    with Path("notes_collapse_wild_hetero.txt").open("w", encoding="utf-8") as fp:
        for line in notes:
            fp.write(line + "\n")
