...
def parse_args(args: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collapsed event study for high prior illiquidity subset with wild bootstrap.")
    ...
    parser.add_argument("--lookback", type=int, default=12, help="Number of pre-event months to compute prior Amihud.")
...
def select_high_prior(df: pd.DataFrame, lookback: int) -> List[str]:
    pre_window = df[df["k"].between(-lookback, -1)]
    agg = pre_window.groupby("ticker", observed=True)["amihud"].mean().dropna()
    if agg.empty:
        return []
    threshold = agg.median()
    return agg[agg >= threshold].index.tolist()


def filter_panel(df: pd.DataFrame, tickers: List[str], measure: str, kmin: int, kmax: int) -> pd.DataFrame:
    ...
    subset = subset.dropna(subset=list(required))
    subset = subset[subset["k"].between(kmin, kmax)]
...
def fit_collapsed_model(df: pd.DataFrame, baseline: int) -> tuple:
    terms = ["pre"] + POST_GROUPS
    formula = f"cs_spread ~ {' + '.join(terms)} + C(ticker) + C(month)"
...
def wild_cluster_bootstrap(
    ...
    for _ in range(B):
        ...
        df_boot = df.copy()
        df_boot["cs_spread"] = y_star

        boot_model = smf.ols(
            f"cs_spread ~ {' + '.join(terms)} + C(ticker) + C(month)",
            data=df_boot,
        )
        try:
...
def write_notes(path: Path, results: pd.DataFrame) -> None:
    with path.open("w", encoding="utf-8") as fp:
        for term in ("post_0_3", "post_all"):
            row = results.loc[results["term"] == term]
            if row.empty:
                fp.write(f"{term}: coef=nan, p_wild=nan\n")
            else:
                value = row.iloc[0]
                fp.write(f"{term}: coef={value['coef']:.6f}, p_wild={value['p_wild']:.4f}\n")
...
def main(cli_args: Iterable[str] | None = None) -> None:
    ...
    high_prior_tickers = select_high_prior(panel_df, args.lookback)
    if not high_prior_tickers:
        raise RuntimeError("No tickers satisfy the high prior illiquidity criterion.")

    df_subset = filter_panel(panel_df, high_prior_tickers, "cs_spread", args.kmin, args.kmax)
    if df_subset.empty:
        raise RuntimeError("Filtered dataset is empty after applying high prior illiquidity filter.")

    df_design = build_design(df_subset)
    weights = compute_weights(df_design)

    base_result, terms = fit_collapsed_model(df_design, args.baseline)
    boot_t, post_all_t = wild_cluster_bootstrap(df_design, terms, base_result, args.B, args.seed, weights)
    ...
    combined.to_csv("collapse_wild_cs_highprior.csv", index=False)

    write_notes(Path("notes_hetero_highprior.txt"), combined)
